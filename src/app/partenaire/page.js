...
const TYPES_PARTENARIAT = [
  { 
    ...
    icon:"🎓", 
    desc:"École, université, centre de formation professionnelle",
    color:"#2563EB",
  ...
  { 
    ...
    icon:"💼", 
    desc:"Entreprise souhaitant valoriser ses employés ou recruter",
    color:"#7C3AED",
  ...
  { 
    ...
    icon:"🤝", 
    desc:"Consultant, agent, intermédiaire commercial",
    color:"#0EA5E9",
  ...
  { 
    ...
    icon:"🌍", 
    desc:"Organisation à but non lucratif, association",
    color:"#16A34A",
...
export default function PartenaireInscriptionPage() {
  ...
  const v1=()=>{
    const e={};
    if(!form.nomOrganisation.trim()) e.nomOrganisation="Nom de l'organisation requis";
  ...
  const v2=()=>{
    const e={};
    if(!form.nomContact.trim()) e.nomContact="Nom du contact requis";
    ...
    const iv=setInterval(()=>{
      ...
      if(p>=100){
        ...
        setTimeout(() => {
          ...
          router.push(`/success?type=partenaire&nom=${encodeURIComponent(form.nomOrganisation)}`);
        }, 700);
      }
  ...
  if(submitted) return (
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#0B1628,#162F52)",display:"flex",alignItems:"center",justifyContent:"center",padding:"1rem",fontFamily:"system-ui,sans-serif"}}>
      <div style={{maxWidth:480,width:"100%",textAlign:"center"}}>
        ...
        <h1 style={{fontFamily:"'Sora',sans-serif",color:"white",fontWeight:900,fontSize:"1.4rem",marginBottom:".5rem"}}>Demande de partenariat reçue !</h1>
        <p style={{color:"rgba(255,255,255,.68)",fontSize:".88rem",lineHeight:1.7,marginBottom:"1.4rem"}}>
          Notre équipe vous contactera sous <strong style={{color:"#F0C040"}}>48h</strong> pour discuter des modalités de collaboration.
        </p>
        <div style={{background:"rgba(255,255,255,.07)",border:"1px solid rgba(255,255,255,.12)",borderRadius:"16px",padding:"1.1rem",marginBottom:"1.25rem",textAlign:"left"}}>
          ...
          {[
            ["Organisation",form.nomOrganisation],
            ["Type",TYPES_PARTENARIAT.find(t=>t.id===form.typePartenariat)?.label||"—"],
            ["Contact",form.nomContact],
            ["Email",form.email],
            ["Objectifs",`${form.objectifs.length} sélectionné${form.objectifs.length>1?"s":""}`]
          ].map(([k,v])=>(
        ...
        <div style={{display:"flex",gap:".6rem",justifyContent:"center",flexWrap:"wrap"}}>
          <Link href="/partenaires-dashboard" style={{display:"inline-flex",alignItems:"center",gap:".5rem",background:"linear-gradient(135deg,#C9960F,#F0C040)",color:"#0D3B2E",fontWeight:800,fontSize:".9rem",padding:".75rem 1.5rem",borderRadius:"99px",textDecoration:"none",boxShadow:"0 4px 18px rgba(201,150,15,.42)"}}>
            Espace Partenaires →
          </Link>
          <a href={`https://wa.me/${WA_NUM1}?text=${encodeURIComponent("Bonjour TalentProof, je viens de soumettre une demande de partenariat.")}`} target="_blank" rel="noreferrer" style={{display:"inline-flex",alignItems:"center",gap:".45rem",background:"#25D366",color:"white",fontWeight:700,fontSize:".88rem",padding:".75rem 1.4rem",borderRadius:"99px",textDecoration:"none"}}>
    ...
    <div style={{minHeight:"100vh",background:"#F0F4F0",fontFamily:"system-ui,sans-serif"}}>
      ...
      <div style={{maxWidth:600,margin:"0 auto",padding:"1.2rem 1rem"}}>
        ...
        {etape===1&&(
          <div style={{display:"flex",flexDirection:"column",gap:".9rem"}}>
            <div style={{background:"white",borderRadius:"18px",padding:"1.35rem",boxShadow:"0 2px 8px rgba(0,0,0,.06),0 8px 24px rgba(0,0,0,.07)"}}>
              ...
              {errors.typePartenariat&&<div style={{color:"#EF4444",fontSize:".77rem",marginBottom:".65rem",background:"#FEF2F2",padding:".42rem .8rem",borderRadius:"8px",border:"1px solid #FCA5A5"}}>{errors.typePartenariat}</div>}
              <div style={{display:"flex",flexDirection:"column",gap:".7rem"}}>
                {TYPES_PARTENARIAT.map(type=>(
                  <button 
                    key={type.id} 
                    onClick={()=>set("typePartenariat",type.id)} 
                    style={{
                      display:"flex",
                      alignItems:"center",
                      gap:".9rem",
                      border:`2px solid ${form.typePartenariat===type.id?type.color:"#E5E7EB"}`,
                      borderRadius:"14px",
                      padding:"1rem",
                      cursor:"pointer",
                      textAlign:"left",
                      transition:"all .2s",
                      background:"white",
                      boxShadow:form.typePartenariat===type.id?`0 0 0 3px ${type.color}22`:"none"
                    }}
                  >
              ...
              <div style={{display:"flex",flexDirection:"column",gap:".82rem"}}>
                <Champ label="Nom de l'organisation" required error={errors.nomOrganisation}>
                  <input 
                    ...
                    onChange={e=>set("nomOrganisation",clean(e.target.value))} 
                    placeholder="Ex: CFPT Abidjan" 
                    style={{...inputSt,borderColor:errors.nomOrganisation?"#EF4444":"#D1FAE5"}} 
                    onFocus={e=>e.target.style.borderColor="#1B6B47"} 
                    onBlur={e=>e.target.style.borderColor=errors.nomOrganisation?"#EF4444":"#D1FAE5"}
                  />
                </Champ>
        ...
        {etape===3&&(
          <div style={{display:"flex",flexDirection:"column",gap:".9rem"}}>
            <div style={{background:"white",borderRadius:"18px",padding:"1.35rem",boxShadow:"0 2px 8px rgba(0,0,0,.06),0 8px 24px rgba(0,0,0,.07)"}}>
              <h2 style={{fontFamily:"'Sora',sans-serif",fontWeight:800,fontSize:".98rem",color:"#111",marginBottom:".35rem"}}>Vos objectifs de partenariat</h2>
              <p style={{color:"#666",fontSize:".8rem",marginBottom:"1rem",lineHeight:1.6}}>
                Sélectionnez un ou plusieurs objectifs (plusieurs choix possibles).
              </p>
              ...
              <Champ label="Objectifs" required error={errors.objectifs}>
                  ...
                  {OBJECTIFS.map(obj=>(
                    <button 
                      ...
                      style={{
                        padding:".4rem .85rem",
                        borderRadius:"99px",
                        border:`1.5px solid ${form.objectifs.includes(obj)?"#1B6B47":"#E5E7EB"}`,
                        background:form.objectifs.includes(obj)?"#1B6B4718":"white",
                        color:form.objectifs.includes(obj)?"#1B6B47":"#666",
                        fontWeight:600,
                        fontSize:".77rem",
                        cursor:"pointer",
                        transition:"all .15s"
                ...
                {form.objectifs.length>0&&(
                  <div style={{marginTop:".5rem",fontSize:".75rem",color:"#16A34A",fontWeight:600}}>
                    ✓ {form.objectifs.length} objectif{form.objectifs.length>1?"s":""} sélectionné{form.objectifs.length>1?"s":""}
                  </div>
                )}
              </Champ>
            </div>
            ...
            <div style={{background:"white",borderRadius:"18px",padding:"1.35rem",boxShadow:"0 2px 8px rgba(0,0,0,.06),0 8px 24px rgba(0,0,0,.07)"}}>
              <div style={{display:"flex",flexDirection:"column",gap:".82rem"}}>
                <Champ label="Nombre de talents concernés (estimation)">
                  <input 
                    value={form.nombreTalents} 
                    onChange={e=>set("nombreTalents",e.target.value)} 
                    placeholder="Ex: 50 apprenants par an, 200 employés..." 
                    style={inputSt} 
                    onFocus={e=>e.target.style.borderColor="#1B6B47"} 
                    onBlur={e=>e.target.style.borderColor="#D1FAE5"}
                  />
                ...
                <Champ label="Message ou détails supplémentaires (optionnel)">
                  <textarea 
                    ...
                    placeholder="Ex: Nous formons 150 jeunes par an dans les métiers du BTP et souhaitons leur offrir une visibilité sur TalentProof..." 
                    rows={4} 
                    maxLength={500} 
